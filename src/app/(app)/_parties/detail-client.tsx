          ...
          <تذييل_الحوار>
            ...
            {معرف_الطرف != null && قائمة.length > 0 && (
              <الزر
                variant="blue"
                onClick={() => {
                  // لو الشيكات تابعة لمعاملة استلام/تظهير، نبعت معرّف المعاملة نفسه فيجيب
                  // التقرير كل شيكاتها الحالية من الخادم مباشرة (حتى لو اتضاف شيك بعدين من مكان تاني)
                  // بدل قائمة معرّفات جامدة ممكن تبقى قديمة.
                  const معرف_دفعة = قائمة[0]?.معرف_معاملة;
                  const رابط = معرف_دفعة
                    ? `/cheque-report?partyBatch=${معرف_دفعة}&party=${معرف_الطرف}&type=${نوع_الطرف ?? ""}`
                    : `/cheque-report?ids=${قائمة.map((x) => x.id).join(",")}&party=${معرف_الطرف}&type=${نوع_الطرف ?? ""}`;
                  window.open(رابط, "_blank", "noopener");
                }}
              >
