...
export function حركات_الطرف({
  ...
  // تجميع صفوف الشيكات في صف واحد (إجمالي + عدد) — العرض فقط:
  //  • شيكات لها «معرّف معاملة» (تظهير مورد) تُجمَّع بحسب المعرّف (كل معاملة لوحدها، ولو غير متتالية).
  //  • الشيكات القديمة/الاستلام (بلا معرّف) تُجمَّع بالتتابع كما كان.
  ...
  const معاملات_ظهرت = new Set<number>();
  for (let i = 0; i < حركات_مفلترة.length; ) {
    const ح = حركات_مفلترة[i];
    const معرف_معاملة = ح.شيك_مرتبط?.معرف_معاملة ?? null;
    // (أ) شيك ضمن معاملة تظهير محدّدة → اجمع كل أعضائها (ولو غير متتالية)، عند أول ظهور (الأحدث)
    if (ح.شيك_مرتبط && معرف_معاملة != null) {
      if (معاملات_ظهرت.has(معرف_معاملة)) { i++; continue; }
      معاملات_ظهرت.add(معرف_معاملة);
      const أعضاء = حركات_مفلترة.filter((x) => x.شيك_مرتبط && x.شيك_مرتبط.معرف_معاملة === معرف_معاملة);
      حركات_معروضة.push(أعضاء.length >= 2 ? بناء_مجموعة(أعضاء, معرف_معاملة) : ح);
      i++;
      continue;
    }
    // (ب) شيكات قديمة/استلام بلا معرّف → تجميع متتابع
    if (ح.شيك_مرتبط) {
      const مجموعة: حركة[] = [];
      let j = i;
      while (j < حركات_مفلترة.length && حركات_مفلترة[j].شيك_مرتبط && (حركات_مفلترة[j].شيك_مرتبط!.معرف_معاملة ?? null) == null) { مجموعة.push(حركات_مفلترة[j]); j++; }
      ...
      {إضافة && معرف_المورد != null && (
        <حوار_إضافة_شيكات_للمعاملة
          ...
          عند_الإضافة={(مضافة, معرف_معاملة_ناتج) => {
            // ألحق الشيكات المضافة ووحّد معرّف المعاملة على الكل (حالتها الآن «مظهّر»)
            تعيين_قائمة((ح) => [
...
function حوار_إضافة_شيكات_للمعاملة({
  ...
  async function أضف() {
    ...
    تعيين_جارٍ(true);
    const r = await ظهّر_شيكات_لمورد(معرف_المورد, [...مختارة], معرف_معاملة, معرفات_المعاملة);
    تعيين_جارٍ(false);
    if (!r.نجاح) return إشعار.خطأ(r.رسالة);
