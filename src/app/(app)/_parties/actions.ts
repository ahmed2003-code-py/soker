...
/**
 * تسجيل دفعة (الجانب الخاص بالطرف فقط في هذه المرحلة).
 * عميل: قيد دائن (يقلّل مديونيته). مورد: قيد مدين (يقلّل المستحق له).
 * ⚠️ نقطة تكامل (المرحلة 9): الجانب الخاص بالخزنة (إيراد/مصروف) يُربط هنا
 * عبر الخدمة الموحّدة دون تكرار المنطق.
 */
export async function سجل_دفعة(مدخلات: unknown): Promise<نتيجة> {
  const فاعل = await اطلب_المستخدم();
  تحقق_الصلاحية(فاعل.role, "كتابة");
  ...
  const عميل = طرف.type === PartyType.CUSTOMER;
  await prisma.$transaction(async (tx) => {
    const قيد = await أضف_قيد(tx, {
      معرف_الطرف: طرف.id,
      التاريخ: تاريخ,
      البيان: عميل
        ? `تحصيل دفعة — ${ب.طريقة_الدفع}`
        : `صرف دفعة للمورد — ${ب.طريقة_الدفع}`,
      مدين: عميل ? 0 : ب.المبلغ!,
      دائن: عميل ? ب.المبلغ! : 0,
    ...
    await تسجيل_عملية(tx, {
      ...
      معرف_الكيان: قيد.id,
      التفاصيل: { دفعة: ب.المبلغ, طريقة: ب.طريقة_الدفع, طرف: طرف.name },
    });
  });

  // تحذير الدفع الزائد (يجعل رصيد العميل سالباً)
  const بعد = await prisma.party.findUnique({ where: { id: طرف.id } });
  const زائد = عميل && بعد && Number(بعد.balance) < 0;

  revalidatePath(`${مسار_الطرف(طرف.type)}/${طرف.id}`);
  return نجح(
    undefined,
    زائد ? "تم تسجيل الدفعة (تنبيه: الرصيد أصبح دفعة مقدمة/سالباً)" : "تم تسجيل الدفعة"
  );
}
