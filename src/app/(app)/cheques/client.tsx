...
export type شيك = {
  ...
  تاريخ_التظهير?: string | null;   // تاريخ التظهير للمورد
...
function يدعم_الافتتاحي_ش(ص: شيك): boolean {
  return نموذج_جديد_ش(ص) || ص.الاتجاه === "OUTGOING";
}
...
export function شاشة_الشيكات({
  ...
  const [تحويل_شيك, تعيين_تحويل_شيك] = React.useState<{ ش: شيك; إلى: "عادي" | "افتتاحي" } | null>(null);
            ...
            <>
              ...
              {ص.الحالة === "ENDORSED" && ص.اسم_المظهر_له && (
                <span className="text-[11px] text-muted-foreground">
                  {ص.اسم_المظهر_له}
                  {ص.تاريخ_التظهير && <> — <نص_تاريخ القيمة={ص.تاريخ_التظهير} /></>}
                </span>
              )}
            </>
          )}
    ...
    <جدول_بيانات
      ...
      إجراءات_الصف={(ص) => (
        <div className="flex justify-end gap-1">
          ...
          {/* تبديل نوع الشيك (افتتاحي ⇄ عادي) — بتأكيد لتفادي الضغط الخاطئ */}
          {يدعم_الافتتاحي_ش(ص) && (
            <الزر
              size="sm"
              variant="ghost"
              title={ص.افتتاحي
                ? `تحويل لشيك عادي (تسجيله بحساب ${ص.الاتجاه === "INCOMING" ? "العميل" : "المورد"})`
                : "إرجاع لشيك افتتاحي (إزالة أثره من الحساب)"}
              onClick={() => تعيين_تحويل_شيك({ ش: ص, إلى: ص.افتتاحي ? "عادي" : "افتتاحي" })}
            >
              <ArrowLeftRight className={`size-4 ${ص.افتتاحي ? "text-amber-600" : "text-muted-foreground"}`} />
            </الزر>
          )}
    ...
    <div className="space-y-4">
      ...
      {تحويل_شيك && (
        <حوار_تأكيد
          مفتوح
          خطر={false}
          عند_التغيير={(o) => !o && تعيين_تحويل_شيك(null)}
          العنوان={تحويل_شيك.إلى === "عادي" ? "تحويل لشيك عادي" : "إرجاع لشيك افتتاحي"}
          الوصف={
            تحويل_شيك.ش.الاتجاه === "INCOMING"
              ? تحويل_شيك.إلى === "عادي"
                ? "سيُسجَّل الشيك في حساب العميل وتُطبَّق آثاره حسب حالته (دين العميل يقل + حركة البنك/التظهير إن وُجدت)."
                : "سيرجع الشيك افتتاحياً ويُزال أثره من الحسابات (يرجع دين العميل ومستحق المورد وحركة الخزنة إن وُجدت)."
              : تحويل_شيك.إلى === "عادي"
                ? "سيُسجَّل الشيك في حساب المورد وتُطبَّق آثاره حسب حالته (مستحق المورد يقل + حركة صرف من البنك إن كان مصروفاً)."
                : "سيرجع الشيك افتتاحياً ويُزال أثره من الحسابات (يرجع مستحق المورد وحركة الخزنة إن وُجدت)."
          }
          نص_التأكيد={تحويل_شيك.إلى === "عادي" ? "تحويل لعادي" : "إرجاع لافتتاحي"}
          عند_التأكيد={async () => {
            const r = تحويل_شيك.إلى === "عادي" ? await حوّل_شيك_لعادي(تحويل_شيك.ش.id) : await حوّل_شيك_لافتتاحي(تحويل_شيك.ش.id);
            r.نجاح ? إشعار.نجاح(r.رسالة!) : إشعار.خطأ(r.رسالة);
            تعيين_تحويل_شيك(null);
            if (r.نجاح) router.refresh();
          }}
        />
      )}
...
export function حوار_شيك({
  ...
  const [ق, تعيين] = React.useState({
    ...
    // فاضي عند الإضافة — المستخدم لازم يدخله بنفسه، من غير قيمة افتراضية
    تاريخ_الاستحقاق: شيك?.تاريخ_الاستحقاق?.slice(0, 10) ?? "",
  ...
  // بيانات خط الأساس الافتتاحي تُدخَل عند الإضافة فقط (لا تُعدَّل بعد ذلك — تُصحَّح بتبديل النوع)
  const افتتاحي_جديد = افتتاحي && !شيك;

  // اختيار الطرف يربط الشيك ويملأ الاسم المقابل: الوارد ← «محوّل من» (العميل)، الصادر ← «المستفيد» (المورد)
  function اختر_الطرف(id: string) {
    تعيين_معرف_الطرف(id);
  ...
  async function احفظ() {
    ...
    if (!ق.تاريخ_الاستحقاق) { إشعار.خطأ("أدخل تاريخ الاستحقاق"); return; }
    // تحقق بيانات الشيك الافتتاحي حسب حالة الدخول
    if (افتتاحي_جديد && وارد && ق.الحالة === "ENDORSED" && !معرف_مورد_افتتاحي) { إشعار.خطأ("اختر المورد المُظهَّر له الشيك"); return; }
    if (افتتاحي_جديد && (ق.الحالة === "DEPOSITED" || ق.الحالة === "COLLECTED") && !معرف_حساب_افتتاحي) {
      إشعار.خطأ(وارد ? "اختر حساب الخزنة الذي يقيم فيه الشيك" : "اختر الحساب الذي صُرف منه الشيك");
      return;
    }
    تعيين_جارٍ(true);
    const payload = {
      ...
      نص_OCR: صورة?.نص ?? null,
      افتتاحي,
      معرف_مورد_افتتاحي: افتتاحي_جديد && وارد && ق.الحالة === "ENDORSED" && معرف_مورد_افتتاحي ? Number(معرف_مورد_افتتاحي) : null,
      معرف_حساب_افتتاحي: افتتاحي_جديد && (ق.الحالة === "DEPOSITED" || ق.الحالة === "COLLECTED") && معرف_حساب_افتتاحي ? Number(معرف_حساب_افتتاحي) : null,
      معرف_حساب_فرعي_افتتاحي: افتتاحي_جديد && (ق.الحالة === "DEPOSITED" || ق.الحالة === "COLLECTED") && معرف_حساب_فرعي_افتتاحي ? Number(معرف_حساب_فرعي_افتتاحي) : null,
    };
  ...
  return (
    <الحوار open onOpenChange={(o) => !o && عند_الإغلاق()}>
      <محتوى_الحوار className={وضع_دفعة ? "max-w-5xl" : "max-w-2xl"}>
            ...
            {(["INCOMING", "OUTGOING"] as ChequeDirection[]).map((dir) => (
              <button
                ...
                onClick={() => {
                  ...
                  تعيين_معرف_الدفتر("");
                  تعيين_رقم_الورقة("");
                  تعيين_معرف_مورد_افتتاحي("");
                  تعيين_معرف_حساب_افتتاحي("");
                  تعيين_معرف_حساب_فرعي_افتتاحي("");
                }}
              >
                {dir === "INCOMING" ? t("cheque.tab.incoming") : t("cheque.tab.outgoing")}
              </button>
        ...
        {/* شيك افتتاحي (وارد أو صادر): يُسجَّل في الشيكات بلا حركة (القيمة ضمن الرصيد الافتتاحي) */}
        {!شيك && (
          <div className="rounded-lg border border-amber-300 bg-amber-50 p-3 space-y-2">
            <label className="flex items-center gap-2 cursor-pointer select-none">
              <input
                ...
                onChange={(e) => {
                  ...
                  تعيين_افتتاحي(قيمة);
                  // افتراضياً: الوارد الافتتاحي «معي» (REGISTERED)، والصادر الافتتاحي «مسلّم للمورد» (PENDING)
                  if (قيمة) حدّث("الحالة", وارد ? "REGISTERED" : "PENDING");
                }}
              />
            ...
            {افتتاحي && (
              <p className="text-[12px] leading-5 text-amber-800">
                ...
                حساب {وارد ? "العميل" : "المورد"} أو الخزنة عند الإضافة. اختر حالته الحالية وبياناتها؛ أي انتقال
                لاحق ({وارد ? "تحصيل/ارتداد" : "صرف/ارتداد"}/إلخ) يُنشئ حركة حقيقية.
              </p>
            )}
          </div>
        )}
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <div className="space-y-1.5">
            <العنوان مطلوب>{وارد ? "محوّل من (العميل)" : "المورد"}</العنوان>
            <قائمة_اختيار
              autoFocus
              الخيارات={أطراف_مناسبة.map((p) => ({ القيمة: String(p.id), التسمية: p.الاسم }))}
              القيمة={معرف_الطرف}
              عند_التغيير={اختر_الطرف}
              نص_بديل={وارد ? "اختر العميل…" : "اختر المورد…"}
            />
          </div>
          ...
          <div className="space-y-1.5">
            ...
            {افتتاحي && !شيك ? (
              // شيك افتتاحي جديد: اختر الحالة الحالية للشيك (نقطة البداية، بلا حركة)
              <قائمة_اختيار
                الخيارات={(وارد
                  ? ([
                      ...
                      ["BOUNCED", "مرتد"],
                      ["CANCELLED", "ملغي"],
                    ] as [ChequeStatus, string][])
                  : ([
                      ["PENDING", "مسلّم للمورد (لسه ما اتصرفش)"],
                      ["COLLECTED", "مصروف من البنك"],
                      ["BOUNCED", "مرتد"],
                      ["CANCELLED", "ملغي"],
                    ] as [ChequeStatus, string][])
                ).map(([v, l]) => ({ القيمة: v, التسمية: l }))}
                القيمة={ق.الحالة}
                عند_التغيير={(v) => حدّث("الحالة", v)}
                قابل_للبحث={false}
              />
            ) : شيك_v2_وارد || افتتاحي ? (
              // الحالة تُدار بالانتقالات لا بالنموذج (وارد v2، وأي شيك افتتاحي قائم)
              <div className="flex h-10 items-center rounded-lg border border-border bg-appgray px-3 text-sm text-muted-foreground">
                ...
                <span className="mr-2 text-[11px]">
                  — {وارد ? "تتغيّر بالإيداع/التظهير/التحصيل" : "تتغيّر بالصرف/الارتداد"}
                </span>
              </div>
            ) : (
              <قائمة_اختيار
          ...
          {افتتاحي_جديد && وارد && ق.الحالة === "ENDORSED" && (
            <div className="space-y-1.5">
          ...
          {افتتاحي_جديد && (ق.الحالة === "DEPOSITED" || ق.الحالة === "COLLECTED") && (
            <div className="space-y-1.5">
              <العنوان مطلوب>
                {ق.الحالة === "DEPOSITED" ? "البنك المُودَع فيه" : وارد ? "جهة التحصيل" : "الحساب المصروف منه"}
              </العنوان>
              <قائمة_اختيار
