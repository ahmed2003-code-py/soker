...
export async function إنشاء_فاتورة(مدخلات: unknown): Promise<نتيجة<{ id: number; الرقم: number }>> {
  ...
  if (ب.نوع_الفاتورة === "SALE" && طرف.type !== "CUSTOMER") return فشل("فاتورة البيع تتطلب اختيار عميل");
  if (ب.نوع_الفاتورة !== "SALE" && طرف.type !== "SUPPLIER") return فشل("فواتير المورد تتطلب اختيار مورد");
  ...
  const هو_مورد = ب.نوع_الفاتورة !== "SALE";
  ...
  const فاتورة = await prisma.$transaction(async (tx) => {
    ...
    // PURCHASE (جاية) → صرف من الخزنة للمورد
    // SALE أو SUPPLIER_RETURN (رايحة) → تحصيل للخزنة
    if (ب.الدفعة) {
