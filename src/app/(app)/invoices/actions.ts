  ...
  const فاتورة = await prisma.$transaction(async (tx) => {
    ...
    if (ب.الدفعة) {
      await أنشئ_عملية_مرتبطة(tx, {
        ...
        معرف_الفاتورة: f.id,
        البيان: ب.الدفعة.ملاحظات || `تحصيل فاتورة رقم ${رقم} — ${عميل.name}`,
        أنشأ: فاعل.id,
      });
    }
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (ب.الدفعة) {
      await أنشئ_عملية_مرتبطة(tx, {
        ...
        معرف_الفاتورة: id,
        البيان: ب.الدفعة.ملاحظات || `تحصيل فاتورة رقم ${رقم_الجديد} — ${عميل.name}`,
        أنشأ: فاعل.id,
      });
    }
    ...
    await تسجيل_عملية(tx, {
      ...
      التفاصيل: {
        ...
        بعد: { الرقم: رقم_الجديد, الإجمالي: الإجمالي_المالي.toString(), ...(ب.الدفعة ? { دفعة: ب.الدفعة.المبلغ } : {}) },
      },
    });
  });
