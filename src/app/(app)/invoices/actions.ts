...
export async function إنشاء_فاتورة(مدخلات: unknown): Promise<نتيجة<{ id: number; الرقم: number }>> {
  ...
  // فاتورة غير مسعّرة: للبيع لعميل مسجّل/مؤقت فقط، بلا دفعة، وتُحفظ بلا أسعار (بلا أثر مالي)
  const غير_مسعّرة = ب.غير_مسعّرة === true;
  if (غير_مسعّرة) {
    if (هو_مورد) return فشل("الفاتورة غير المسعّرة للبيع للعميل فقط");
    if (!طرف && !وضع_مؤقت) return فشل("الفاتورة غير المسعّرة تتطلب عميلاً مسجّلاً (تُحفظ على حسابه)");
    if (ب.الدفعة) return فشل("لا يمكن تحصيل دفعة على فاتورة غير مسعّرة — سعّرها أولاً");
  }
  ...
  // غير مسعّرة: تُجرَّد الأسعار فتصبح كل الإجماليات صفراً (بلا قيد على العميل)
  const بنود_الإدخال = غير_مسعّرة ? ب.البنود.map((x) => ({ ...x, السعر: null })) : ب.البنود;
  const { إجمالي_الكمية, إجمالي_الوزن, الإجمالي_المالي, إجمالي_المبيعات, إجمالي_المرتجعات, بنود_محسوبة } =
    احسب_إجماليات(بنود_الإدخال);
...
export async function تعديل_فاتورة(id: number, مدخلات: unknown): Promise<نتيجة> {
  ...
  if (غير_مسعّرة) {
    if (هو_مورد) return فشل("الفاتورة غير المسعّرة للبيع للعميل فقط");
    if (!طرف) return فشل("الفاتورة غير المسعّرة تتطلب عميلاً مسجّلاً (تُحفظ على حسابه)");
    if (ب.الدفعة) return فشل("لا يمكن تحصيل دفعة على فاتورة غير مسعّرة — سعّرها أولاً");
  } else if (!طرف && !ب.الدفعة) return فشل("العميل الزائر يتطلب تحصيل فوري");
