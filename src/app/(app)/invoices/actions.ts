...
export async function إنشاء_فاتورة(مدخلات: unknown): Promise<نتيجة<{ id: number; الرقم: number }>> {
  ...
  const ب = t.data;
  const طرف = await prisma.party.findUnique({ where: { id: ب.معرف_العميل } });
  if (!طرف) return فشل("الطرف غير موجود");
  // التحقق من تطابق نوع الفاتورة مع نوع الطرف
  const أنواع_العميل = ["SALE", "CUSTOMER_RETURN"];
  const أنواع_المورد = ["PURCHASE", "SUPPLIER_RETURN"];
  if (أنواع_العميل.includes(ب.نوع_الفاتورة) && طرف.type !== "CUSTOMER") return فشل("هذا النوع من الفواتير يتطلب اختيار عميل");
  if (أنواع_المورد.includes(ب.نوع_الفاتورة) && طرف.type !== "SUPPLIER") return فشل("هذا النوع من الفواتير يتطلب اختيار مورد");
  ...
  const فاتورة = await prisma.$transaction(async (tx) => {
    ...
    } else if (ب.نوع_الفاتورة === "CUSTOMER_RETURN") {
    ...
    // PURCHASE → صرف من الخزنة للمورد
    // الباقي → تحصيل (عميل يدفع أو مرتجع مع دفع نقدي)
    if (ب.الدفعة) {
      const اتجاه_الدفعة = ب.نوع_الفاتورة === "PURCHASE" ? "صرف" : "تحصيل";
      const بيان_الدفعة = ب.الدفعة.ملاحظات ||
        (اتجاه_الدفعة === "تحصيل"
          ? ب.نوع_الفاتورة === "CUSTOMER_RETURN"
            ? `تحصيل مرتجع رقم ${رقم} — ${طرف.name}`
  ...
  if (t.data.الدفعة) revalidatePath("/treasury");
...
export async function تعديل_فاتورة(id: number, مدخلات: unknown): Promise<نتيجة> {
  ...
  const طرف = await prisma.party.findUnique({ where: { id: ب.معرف_العميل } });
  if (!طرف) return فشل("الطرف غير موجود");
  const أنواع_العميل_ت = ["SALE", "CUSTOMER_RETURN"];
  const أنواع_المورد_ت = ["PURCHASE", "SUPPLIER_RETURN"];
  if (أنواع_العميل_ت.includes(ب.نوع_الفاتورة) && طرف.type !== "CUSTOMER") return فشل("هذا النوع يتطلب اختيار عميل");
  if (أنواع_المورد_ت.includes(ب.نوع_الفاتورة) && طرف.type !== "SUPPLIER") return فشل("هذا النوع يتطلب اختيار مورد");
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (ب.الدفعة) {
      ...
      const بيان_الدفعة = ب.الدفعة.ملاحظات ||
        (اتجاه_الدفعة === "تحصيل"
          ? ب.نوع_الفاتورة === "CUSTOMER_RETURN"
            ? `تحصيل مرتجع رقم ${رقم_الجديد} — ${طرف.name}`
    ...
    await تسجيل_عملية(tx, {
      ...
      التفاصيل: {
        ...
        بعد: { الرقم: رقم_الجديد, النوع: ب.نوع_الفاتورة, الإجمالي: الإجمالي_المالي.toString(), ...(ب.الدفعة ? { دفعة: ب.الدفعة.المبلغ } : {}) },
      },
    });
  });
  ...
  if (هو_مورد) {
    revalidatePath(`/suppliers/${طرف.id}`);
    revalidatePath(`/suppliers/${حالية.customerId}`);
  } else {
    revalidatePath(`/customers/${طرف.id}`);
