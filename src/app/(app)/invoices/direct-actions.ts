 ...
 *  - فاتورة شراء على المورد  → قيد دائن بقيمة الشراء (سعر المورد)
 *  - فاتورة بيع على العميل   → قيد مدين بقيمة البيع (سعر العميل)
 * لكل جهة سعرها الخاص، والفرق بين القيمتين = ربح المعاملة.
...
async function جهّز(مدخلات: unknown) {
  ...
  // إجماليات لكل جهة بسعرها: المورد بسعر الشراء، العميل بسعر البيع
  const إجماليات_المورد = احسب_إجماليات(بنود.map((x) => ({ ...x, السعر: x.سعر_المورد })));
  const إجماليات_العميل = احسب_إجماليات(بنود);

  // غير مسعّرة: أي بند ناقصه سعر (لأي جهة) ⇒ الفاتورتان تُحفظان بلا أثر مالي حتى تكتمل الأسعار
  const غير_مسعّرة = بنود.some((x) => x.السعر == null || x.سعر_المورد == null);
  ...
  // الكمية/الوزن واحدة للجهتين (نفس البضاعة) — بناخدها من جهة العميل
  return {
    سليم: true, ب, مورد, عميل, تاريخ, غير_مسعّرة,
    إجمالي_الكمية: إجماليات_العميل.إجمالي_الكمية,
    إجمالي_الوزن: إجماليات_العميل.إجمالي_الوزن,
    قيمة_المورد: إجماليات_المورد.الإجمالي_المالي,
    قيمة_العميل: إجماليات_العميل.الإجمالي_المالي,
    بنود_المورد: إجماليات_المورد.بنود_محسوبة,
    بنود_العميل: إجماليات_العميل.بنود_محسوبة,
  } as const;
}
  ...
  const نتيجة_المعاملة = await prisma.$transaction(async (tx) => {
    ...
    const فاتورة_المورد = await tx.invoice.create({
      data: {
        ...
        totalAmount: ج.قيمة_المورد,
        notes: ب.ملاحظات || null,
        shareToken: crypto.randomUUID(),
        directInvoiceId: مجموعة.id,
        createdById: فاعل.id,
        lines: { create: بنود_للإنشاء(ج.بنود_المورد, فاعل.id) },
      },
    });
    ...
    const فاتورة_العميل = await tx.invoice.create({
      data: {
        ...
        totalQty: ج.إجمالي_الكمية,
        totalWeight: ج.إجمالي_الوزن,
        totalAmount: ج.قيمة_العميل,
        notes: ب.ملاحظات || null,
        shareToken: crypto.randomUUID(),
        directInvoiceId: مجموعة.id,
        createdById: فاعل.id,
        lines: { create: بنود_للإنشاء(ج.بنود_العميل, فاعل.id) },
      },
    });

    // ── القيود على الحسابين (تُمنَع طالما الفاتورة غير مسعّرة) ──
    if (!غير_مسعّرة && (ج.قيمة_المورد.greaterThan(0) || ج.قيمة_العميل.greaterThan(0))) {
      await رحّل_فاتورة_مباشرة(tx, {
        ...
        مرجع_المورد,
        التاريخ: تاريخ,
        قيمة_المورد: ج.قيمة_المورد,
        قيمة_العميل: ج.قيمة_العميل,
        أنشأ: فاعل.id,
      });
    }
    ...
    for (const [معرف, جهة] of [
      ...
      await تسجيل_عملية(tx, {
        ...
        التفاصيل: {
          ...
          المورد: مورد.name,
          العميل: عميل.name,
          قيمة_المورد: ج.قيمة_المورد.toString(),
          قيمة_العميل: ج.قيمة_العميل.toString(),
          الربح: ج.قيمة_العميل.minus(ج.قيمة_المورد).toString(),
          ...(ب.دفعة_العميل ? { تحصيل_من_العميل: ب.دفعة_العميل.بنود.reduce((س, بند) => س + Number(بند.المبلغ), 0) } : {}),
          ...(ب.دفعة_المورد ? { دفع_للمورد: ب.دفعة_المورد.بنود.reduce((س, بند) => س + Number(بند.المبلغ), 0) } : {}),
        },
      });
    }
...
async function سجّل_الدفعات(
  tx: Parameters<typeof سجّل_دفعة_فاتورة>[0],
  م: {
  ...
  if (م.ب.دفعة_العميل) {
    const إجمالي = م.ب.دفعة_العميل.بنود.reduce((س, بند) => س.plus(د(بند.المبلغ!)), د(0));
    await سجّل_دفعة_فاتورة(tx, {
      الاتجاه: "تحصيل",
      طرف: { id: م.عميل.id, name: م.عميل.name },
      بنود: م.ب.دفعة_العميل.بنود.map((بند) => ({
  ...
  if (م.ب.دفعة_المورد) {
    const إجمالي = م.ب.دفعة_المورد.بنود.reduce((س, بند) => س.plus(د(بند.المبلغ!)), د(0));
    await سجّل_دفعة_فاتورة(tx, {
      الاتجاه: "صرف",
      طرف: { id: م.مورد.id, name: م.مورد.name },
      بنود: م.ب.دفعة_المورد.بنود.map((بند) => ({
        معرف_الحساب: بند.معرف_الحساب,
        معرف_حساب_فرعي: بند.معرف_حساب_فرعي ?? null,
        المبلغ: د(بند.المبلغ!),
      })),
