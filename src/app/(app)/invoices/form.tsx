...
export function نموذج_فاتورة({
  ...
  // ─── فاتورة غير مسعّرة (بلا أسعار، بلا أثر مالي حتى تُسعَّر) ───
  const [غير_مسعّرة, تعيين_غير_مسعّرة] = React.useState<boolean>(!!فاتورة?.غير_مسعّرة);
  ...
  async function احفظ() {
    ...
    if (غير_مسعّرة && (عميل_زائر || نوع_الطرف === "SUPPLIER")) return إشعار.خطأ("الفاتورة غير المسعّرة للبيع لعميل مسجّل فقط");
    if (!غير_مسعّرة && عميل_زائر && !دفعة_مفعلة) return إشعار.خطأ("العميل الزائر يتطلب تحصيل فوري — فعّل الدفعة");
    ...
    const payload = {
      ...
      غير_مسعّرة: غير_مسعّرة || undefined,
      ...(!غير_مسعّرة && دفعة_مفعلة && مبلغ_الدفعة && حساب_الدفعة ? {
        الدفعة: {
          المبلغ: مبلغ_الدفعة,
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ── فاتورة غير مسعّرة ── (بيع لعميل مسجّل فقط) */}
      {نوع_الطرف === "CUSTOMER" && !عميل_زائر && (
        <div className="card-soft border-amber-300 bg-amber-50 p-4">
          <label className="flex cursor-pointer items-center gap-2.5 select-none">
            <input
              type="checkbox"
              checked={غير_مسعّرة}
              onChange={(e) => { تعيين_غير_مسعّرة(e.target.checked); if (e.target.checked) تعيين_دفعة_مفعلة(false); }}
              className="size-4 rounded accent-amber-600"
            />
            <span className="font-medium text-amber-900">فاتورة غير مسعّرة (بدون أسعار)</span>
          </label>
          {غير_مسعّرة && (
            <p className="mt-2 text-[12px] leading-5 text-amber-800">
              تُحفظ الفاتورة على حساب العميل بالأصناف والكميات والأوزان فقط، **بدون أي أثر مالي**. الأسعار تُتجاهَل الآن،
              ولا يمكن تحصيل دفعة. لمّا ترجع تسعّرها (تعديل الفاتورة → إلغاء العلامة وإدخال الأسعار) يتحسب أثرها على الحساب.
            </p>
          )}
        </div>
      )}
