...
import { اجلب_لطات_متاحة, اجلب_أصناف_المخزن, type شركة_مخزن } from "@/app/(app)/inventory/actions";
...
export function نموذج_فاتورة({
  ...
  const [لطات_مخزن, تعيين_لطات_مخزن] = React.useState<
    Record<string, { id: number; رقم_اللط: string; الكمية: number; الوزن: number; المورد: string | null }[]>
  >({});

  // جلب اللطات المتاحة لكل (صنف × لون) مستخدَم في البنود
  React.useEffect(() => {
    if (!يختار_لط) return;
    const مفاتيح = [...new Set(
      بنود
        .filter((ب) => ب.التصنيف.trim() && ب.اللون.trim())
        .map((ب) => `${ب.الشركة || ""}||${ب.التصنيف}||${ب.اللون}`)
    )].filter((م) => !(م in لطات_مخزن));
    if (!مفاتيح.length) return;
    for (const م of مفاتيح) {
      const [شركة, تصنيف, لون] = م.split("||");
      اجلب_لطات_متاحة(تصنيف, لون, شركة || null)
        .then((r) => {
          تعيين_لطات_مخزن((prev) => ({ ...prev, [م]: r }));
          // اقتراح FIFO: أقدم لط يتحدد تلقائياً لأي سطر لسه ما اختارش (والمستخدم يقدر يغيّره)
          if (r.length) {
            تعيين_بنود((س) =>
              س.map((ب) =>
                `${ب.الشركة || ""}||${ب.التصنيف}||${ب.اللون}` === م && !ب.معرف_اللط
                  ? { ...ب, معرف_اللط: String(r[0].id) }
                  : ب
              )
            );
            // لو التنقل مستني خانة اللط، ركّز عليها أول ما تجهز
            const مستني = تركيز_معلّق.current;
            if (مستني?.حقل === "لط") {
              requestAnimationFrame(() => ركّز_عند_الجاهزية(مستني.صف, "لط"));
            }
          }
        })
        .catch(() => {});
  ...
  // كتالوج المخزن بترتيب الإدخال: الشركة ← الصنف ← اللون
  const [كتالوج_المخزن, تعيين_كتالوج_المخزن] = React.useState<شركة_مخزن[]>([]);
  React.useEffect(() => {
    if (!يختار_لط) return;
    اجلب_أصناف_المخزن().then(تعيين_كتالوج_المخزن).catch(() => {});
  }, [يختار_لط]);
  ...
  const مفتاح_البند = (ب: بند) => `${ب.الشركة || ""}||${ب.التصنيف}||${ب.اللون}`;
  const لطات_البند = (ب: بند) => لطات_مخزن[مفتاح_البند(ب)] ?? [];
  /** لسه بيحمّل لطات البند من الخادم؟ (عشان ما نقولش «مفيش رصيد» بالغلط) */
  const يحمّل_اللطات = (ب: بند) =>
    !!ب.التصنيف.trim() && !!ب.اللون.trim() && !(مفتاح_البند(ب) in لطات_مخزن);
