...
/** صف وسيلة دفع واحدة ضمن دفعة فورية قد تُوزَّع على أكثر من وسيلة (حتى 4). */
type بند_دفعة = { معرف_محلي: number; حساب: string; حساب_فرعي: string; مبلغ: string };
...
export function نموذج_فاتورة({
  ...
  فاتورة?: {
    ...
    دفعة_المورد?: { بنود: { المبلغ: number; معرف_الحساب: number; معرف_حساب_فرعي: number | null }[] } | null;
    ...
    ملاحظات: string | null;
    غير_مسعّرة?: boolean;
    دفعة?: { بنود: { المبلغ: number; معرف_الحساب: number; معرف_حساب_فرعي: number | null }[] } | null;
    البنود: بند[];
  };
}) {
  ...
  // ─── الدفعة الفورية (وسيلة واحدة أو موزّعة على أكثر من وسيلة) ───
  ...
  const عداد_بند_دفعة = React.useRef(0);
  const بند_دفعة_جديد = React.useCallback((): بند_دفعة => ({
    معرف_محلي: ++عداد_بند_دفعة.current,
    حساب: حسابات_الخزنة[0] ? String(حسابات_الخزنة[0].id) : "",
    حساب_فرعي: "",
    مبلغ: "",
  }), [حسابات_الخزنة]);
  const [بنود_الدفعة, تعيين_بنود_الدفعة] = React.useState<بند_دفعة[]>(() =>
    دفعة_محمّلة?.بنود?.length
      ? دفعة_محمّلة.بنود.map((ب) => ({
          معرف_محلي: ++عداد_بند_دفعة.current,
  ...
  function حدّث_بند_دفعة(معرف_محلي: number, تعديل: Partial<بند_دفعة>) {
    تعيين_بنود_الدفعة((س) => س.map((ب) => (ب.معرف_محلي === معرف_محلي ? { ...ب, ...تعديل } : ب)));
  }
  function احذف_بند_دفعة(معرف_محلي: number) {
    تعيين_بنود_الدفعة((س) => (س.length > 1 ? س.filter((ب) => ب.معرف_محلي !== معرف_محلي) : س));
  }
  /** الصفوف المكتملة فعلاً (فيها مبلغ وحساب) — جاهزة للإرسال للسيرفر. */
  function بنود_الدفعة_للإرسال() {
    return بنود_الدفعة
      .filter((ب) => ب.مبلغ && ع(ب.مبلغ) > 0 && ب.حساب)
      .map((ب) => ({
        المبلغ: ب.مبلغ,
  ...
  function نوع_حساب_دفعة(معرف: string) {
    return حسابات_الخزنة.find((h) => String(h.id) === معرف)?.النوع ?? null;
  }
  const الحد_الأقصى_لوسائل_الدفع = Math.min(4, حسابات_الخزنة.length || 1);
  const إجمالي_بنود_الدفعة = بنود_الدفعة.reduce((س, ب) => س + (ع(ب.مبلغ) || 0), 0);
  /** يتحقق أن كل بند دفعة له مبلغ فيه اختار الحساب الفرعي المطلوب (لو محتاج) — يتجاهل الصفوف الفارغة. */
  function تحقق_بنود_الدفعة(): string | null {
    for (const ب of بنود_الدفعة) {
      if (!ب.مبلغ || ع(ب.مبلغ) <= 0) continue;
      const نوع = نوع_حساب_دفعة(ب.حساب);
      if (نوع && نوع !== "CASH") {
        const خيارات = حسابات_فرعية[نوع] ?? [];
        if (خيارات.length > 0 && !ب.حساب_فرعي) {
          return `يرجى اختيار ${نوع === "BANK" ? "البنك" : نوع === "VODAFONE" ? "المحفظة" : "حساب إنستا"} لوسيلة الدفع`;
        }
      }
    }
    return null;
  ...
  // ─── دفعة المورد (الوضع المباشر فقط) — مستقلة عن تحصيل العميل، وسيلة واحدة ───
  ...
  const [مبلغ_دفعة_المورد, تعيين_مبلغ_دفعة_المورد] = React.useState(
    دفعة_مورد_محمّلة ? String(دفعة_مورد_محمّلة.بنود.reduce((س, ب) => س + ب.المبلغ, 0)) : ""
  );
  const [حساب_دفعة_المورد, تعيين_حساب_دفعة_المورد] = React.useState(
    دفعة_مورد_محمّلة?.بنود[0] ? String(دفعة_مورد_محمّلة.بنود[0].معرف_الحساب) : (حسابات_الخزنة[0] ? String(حسابات_الخزنة[0].id) : "")
  );
  const [حساب_فرعي_دفعة_المورد, تعيين_حساب_فرعي_دفعة_المورد] = React.useState(
    دفعة_مورد_محمّلة?.بنود[0]?.معرف_حساب_فرعي ? String(دفعة_مورد_محمّلة.بنود[0].معرف_حساب_فرعي) : ""
  );
  ...
  React.useEffect(() => {
    تعيين_بنود_الدفعة((س) => {
      ...
      const جديد = س.map((ب) => {
        const نوع = نوع_حساب_دفعة(ب.حساب);
        const خيارات = نوع && نوع !== "CASH" ? (حسابات_فرعية[نوع] ?? []) : [];
        if (خيارات.length === 1 && !ب.حساب_فرعي) {
          تغيّر = true;
          return { ...ب, حساب_فرعي: String(خيارات[0].id) };
        }
        return ب;
      });
      return تغيّر ? جديد : س;
  ...
  }, [بنود_الدفعة.map((ب) => ب.حساب).join("|"), حسابات_فرعية]);
  ...
  async function احفظ_مباشرة() {
    ...
    if (دفعة_مفعلة) {
      const خطأ_بند = تحقق_بنود_الدفعة();
      if (خطأ_بند) return إشعار.خطأ(`${خطأ_بند} (تحصيل العميل)`);
    }
    ...
    const payload = {
      ...
      ...(!سيُحفظ_غير_مسعّر && دفعة_مفعلة && بنود_الدفعة_للإرسال().length > 0 ? {
        دفعة_العميل: { بنود: بنود_الدفعة_للإرسال() },
      } : {}),
  ...
  async function احفظ() {
    ...
    if (دفعة_مفعلة) {
      const خطأ_بند = تحقق_بنود_الدفعة();
      if (خطأ_بند) return إشعار.خطأ(خطأ_بند);
    }
    تعيين_جارٍ(true);
    const رقم_مُحدد = رقم_الفاتورة.trim() ? Number(رقم_الفاتورة.replace(/,/g, "")) : null;
    const payload = {
      ...
      البنود: بنود_للإرسال(),
      ...(!سيُحفظ_غير_مسعّر && دفعة_مفعلة && بنود_الدفعة_للإرسال().length > 0 ? {
        الدفعة: { بنود: بنود_الدفعة_للإرسال() },
      } : {}),
    };
    const r = فاتورة
                        ...
                        <قائمة_اختيار
                          ...
                          عند_التغيير={async (v) => {
                            ...
                            if (عميل && v && !أسعار_تصنيفات[v]) {
                              ...
                              // نحدّث كل بنود نفس التصنيف مش بس هذا البند — لو أكتر من بند
                              // اختار نفس التصنيف قبل ما الجلب يخلص، تتحدّث أسعارهم كلهم مرة واحدة
                              // بدل ما يفضل بعضهم من غير سعر (وتفضل الفاتورة «غير مسعّرة» بغلط).
                              if (أسعار[v]) حدّث_سعر_تصنيف(v, أسعار[v]);
                            }
                          }}
                          {...(يختار_لط
                            ? {}
        ...
        <div className="card-soft space-y-2 p-5">
          ...
          {(() => {
            ...
            if (مباشرة) {
              ...
              if (!م && !ع_) return null;
              const دفعة_عميل = دفعة_مفعلة ? إجمالي_بنود_الدفعة : 0;
            ...
            const مبلغ_دفعة_فعلي = دفعة_مفعلة ? إجمالي_بنود_الدفعة : 0;
        ...
        <label className="flex cursor-pointer items-center gap-2.5 select-none">
          <input
            ...
            onChange={(e) => {
              ...
              if (e.target.checked && بنود_الدفعة.length === 1 && !بنود_الدفعة[0].مبلغ && الإجمالي_المالي > 0) {
                تعيين_بنود_الدفعة((س) => [{ ...س[0], مبلغ: String(الإجمالي_المالي) }]);
              }
            }}
            className="size-4 rounded accent-primary"
          />
        ...
        {دفعة_مفعلة && (
          <div className="mt-3 space-y-2 border-t border-border pt-3">
                ...
                <div key={ب.معرف_محلي} className="flex flex-wrap items-end gap-2 rounded-lg border border-border p-2">
                  <div className="flex-1 min-w-[110px] space-y-1">
                    ...
                    <الحقل
                      ...
                      className="ltr-nums"
                      value={ب.مبلغ}
                      onChange={(e) => حدّث_بند_دفعة(ب.معرف_محلي, { مبلغ: e.target.value })}
                      placeholder="0.00"
                    />
                  </div>
                  <div className="flex-1 min-w-[130px] space-y-1">
                    {i === 0 && <العنوان مطلوب className="mb-0">طريقة الدفع / الخزنة</العنوان>}
                    <قائمة_اختيار
                      الخيارات={حسابات_الخزنة.map((h) => ({ القيمة: String(h.id), التسمية: h.التسمية }))}
                      القيمة={ب.حساب}
                      عند_التغيير={(v) => حدّث_بند_دفعة(ب.معرف_محلي, { حساب: v, حساب_فرعي: "" })}
                      نص_بديل="اختر"
                      قابل_للبحث={false}
                    />
                  </div>
                  {خيارات_فرعية.length > 0 && (
                    <div className="flex-1 min-w-[120px] space-y-1">
                      {i === 0 && (
                        <العنوان مطلوب className="mb-0">
                          {نوع === "BANK" ? "البنك" : نوع === "VODAFONE" ? "المحفظة" : "حساب إنستا"}
                        </العنوان>
                      )}
                      <قائمة_اختيار
                        الخيارات={خيارات_فرعية.map((s) => ({ القيمة: String(s.id), التسمية: s.الاسم }))}
                        القيمة={ب.حساب_فرعي}
                        عند_التغيير={(v) => حدّث_بند_دفعة(ب.معرف_محلي, { حساب_فرعي: v })}
                        نص_بديل="اختر…"
                      />
                    </div>
                  )}
                  {بنود_الدفعة.length > 1 && (
                    <الزر
                      type="button"
                      size="sm"
                      variant="ghost"
                      onClick={() => احذف_بند_دفعة(ب.معرف_محلي)}
            ...
            <div className="flex items-center justify-between">
              {بنود_الدفعة.length < الحد_الأقصى_لوسائل_الدفع ? (
                <الزر
                  type="button"
                  size="sm"
                  variant="outline"
                  onClick={() => تعيين_بنود_الدفعة((س) => [...س, بند_دفعة_جديد()])}
                >
                  <Plus className="size-4" /> وسيلة دفع أخرى
                </الزر>
              ) : <span />}
              {بنود_الدفعة.length > 1 && (
                <div className="flex items-center gap-1.5 text-sm">
                  <span className="text-muted-foreground">الإجمالي:</span>
                  <نص_مبلغ القيمة={إجمالي_بنود_الدفعة} />
                </div>
              )}
            </div>
          </div>
