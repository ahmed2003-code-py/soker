...
/**
 * دفعة فاتورة فورية (عند الإنشاء أو التعديل) — وسيلة دفع واحدة أو عدة وسائل معاً.
 * وسيلة واحدة: نفس المسار المعتاد (عملية مرتبطة لطرف مسجّل، أو حركة خزنة مباشرة للزائر بلا قيد).
 * أكثر من وسيلة: دفعة موزّعة (SplitPayment) بقيد واحد فقط على الطرف (لو مسجّل).
 */
export async function سجّل_دفعة_فاتورة(
  tx: عميل_معاملة,
  ب: {
    الاتجاه: اتجاه;
    طرف: { id: number; name: string } | null; // مسجّل/مؤقت — أو null لعميل زائر بلا حساب
    اسم_الطرف_الخارجي?: string | null;
    بنود: { معرف_الحساب: number; معرف_حساب_فرعي?: number | null; المبلغ: Prisma.Decimal.Value }[];
    الإجمالي: Prisma.Decimal.Value;
    التاريخ: Date;
    البيان?: string | null;
    رقم_الفاتورة?: string | null;
  ...
  if (ب.بنود.length === 1) {
    const بند = ب.بنود[0];
    if (ب.طرف) {
      return أنشئ_عملية_مرتبطة(tx, {
        الاتجاه: ب.الاتجاه,
        معرف_الطرف: ب.طرف.id,
        اسم_الطرف: ب.طرف.name,
        المبلغ: بند.المبلغ,
        التاريخ: ب.التاريخ,
        معرف_الحساب: بند.معرف_الحساب,
        معرف_حساب_فرعي: بند.معرف_حساب_فرعي ?? null,
        رقم_الفاتورة: ب.رقم_الفاتورة ?? null,
        معرف_الفاتورة: ب.معرف_الفاتورة,
    ...
    const حركة = await أضف_حركة_خزنة(tx, {
      التاريخ: ب.التاريخ,
      النوع: ب.الاتجاه === "تحصيل" ? TxnKind.INCOME : TxnKind.EXPENSE,
      المبلغ: بند.المبلغ,
      معرف_الحساب: بند.معرف_الحساب,
      معرف_حساب_فرعي: بند.معرف_حساب_فرعي ?? null,
      البيان: ب.البيان ?? "",
    ...
    return { معرف_حركة_الخزنة: حركة.id, معرف_القيد: null };
  }

  return أنشئ_دفعة_موزعة(tx, {
    الاتجاه: ب.الاتجاه,
    معرف_الطرف: ب.طرف?.id ?? null,
    اسم_الطرف_الخارجي: ب.اسم_الطرف_الخارجي ?? null,
    اسم_الطرف: ب.طرف?.name ?? ب.اسم_الطرف_الخارجي ?? "عميل زائر",
    الإجمالي: ب.الإجمالي,
    التاريخ: ب.التاريخ,
    البيان: ب.البيان ?? null,
    رقم_الفاتورة: ب.رقم_الفاتورة ?? null,
...
/**
 * يعكس كل حركات الفاتورة المالية الفورية — بغض النظر عن كونها وسيلة دفع واحدة
 * (عملية مرتبطة عادية) أو موزّعة على أكثر من وسيلة (SplitPayment). يُستخدم قبل
 * إعادة تطبيق الدفعة عند التعديل، أو نهائيًا عند حذف الفاتورة.
 */
export async function اعكس_دفعات_الفاتورة(tx: عميل_معاملة, معرف_الفاتورة: number) {
  const حركات = await tx.treasuryTxn.findMany({
    where: { invoiceId: معرف_الفاتورة, deletedAt: null },
    select: { id: true, splitPaymentId: true },
  });
  const مجموعات = new Set<number>();
  for (const ح of حركات) {
    if (ح.splitPaymentId) مجموعات.add(ح.splitPaymentId);
    else await اعكس_عملية_مرتبطة(tx, ح.id);
  }
  for (const معرف_مجموعة of مجموعات) await حذف_دفعة_موزعة(tx, معرف_مجموعة);
}
