 ...
 * فتفقد ترتيب الإدخال مقابل القيود المختومة بوقت فعلي (تظهير/دفعات). نُلصق وقت الإدخال الحالي
 * مع الإبقاء على نفس اليوم (يظل تاريخ العرض كما اختاره المستخدم، لكن الترتيب يصبح بالوقت الحقيقي —
 * فآخر ما أُدخِل يظهر أولاً). يطابق منطق الخزنة.
 */
function وقت_القيد(د2: Date): Date {
...
export async function أضف_قيد(
  ...
  // ختم القيد بوقت الإدخال الحقيقي (لو التاريخ منتصف ليل) ليصبح الترتيب داخل اليوم بترتيب الإدخال
  const تاريخ_القيد = وقت_القيد(بيانات.التاريخ);
  ...
  const في_النهاية = !الأخيرة || تاريخ_القيد >= الأخيرة.date;
